### Hi, I'm Zhihao Guo 👋</h1>
- Currently working at Xiaomi, primarily responsible for Redis middleware, data synchronization tools, and high-availability components.  
- Interested in LLM-related infrastructure and currently conducting in-depth research (learning), such as **LLM inference**, **kv-cache**, and **RL infrastructure**.

### Currently focusing on

- *[sglang-omni](https://github.com/sgl-project/sglang-omni) @Contributor* ：SGLang Omni: High-Performance Multi-Stage Pipeline Framework for Omni Models
- *[AReal](https://github.com/areal-project/AReaL)* @Maintainer：The RL Bridge for LLM-based Agent Applications, Made Simple & Flexible

### Where to find me
- *email*: guozhihaoemail@gmail.com  
- *wechat*：335247945
